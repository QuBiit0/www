...
import { sendMessageToGemini, clearChatHistory } from '../services/apiService';
...
const ChatAssistant: React.FC = () => {
  ...
  const handleClear = async () => {
    if (confirm('Are you sure you want to clear the conversation?')) {
      await clearChatHistory(); // Uses default-session
              ...
              {messages.map((msg, idx) => (
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className={`text-xs text-slate-500 ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
